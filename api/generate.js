...
async function callModel(key,model,prompt,ms){
 ...
 try{
  ...
  if(!r.ok)throw Error(`${model}: ${j?.error?.message||`HTTP ${r.status}`}`);
...
export default async function handler(req,res){
 ...
 const data=req.body?.data,subject=req.body?.subject||"Coding / Robotics";if(!data?.hocVien?.length)return res.status(400).json({error:"Không có dữ liệu học viên."});if(data.hocVien.length>5)return res.status(400).json({error:"Mỗi request tối đa 5 học viên."});
 ...
 const seed=Math.max(0,(Number(req.body?.batch)||1)-1);const tries=[MODELS[seed%MODELS.length],MODELS[(seed+1)%MODELS.length]];let last="";
 for(const model of tries){try{const out=await callModel(key,model,prompt,7000),students=out?.students||{};const missing=data.hocVien.filter(s=>!students[s.tenHocVien]).map(s=>s.tenHocVien);if(missing.length)throw Error(`${model}: thiếu ${missing.join(", ")}`);return res.status(200).json({students,model})}catch(e){last=e.message}}
 return res.status(502).json({error:last||"Gemini lỗi",students:{}});
}